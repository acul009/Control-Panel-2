...
func main() {
	...
	{
		logger = log.New(os.Stderr, "[deploymentsapi] ", log.Ltime)
	}
	...
	{
		deploymentsSvc = deploymentsapi.NewDeployments(logger)
	}

	// Wrap the services in endpoints that can be invoked from other services
	// potentially running in different processes.
	var (
		deploymentsEndpoints *deployments.Endpoints
	)
	{
		deploymentsEndpoints = deployments.NewEndpoints(deploymentsSvc)
	}
	...
	case "development":
		{
			...
			handleHTTPServer(ctx, u, deploymentsEndpoints, &wg, errc, logger, *dbgF)
		}
